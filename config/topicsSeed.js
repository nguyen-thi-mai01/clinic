...
async function seedTopics(models, transaction) {
  try {
    ...
    const topics = [
      ...
      },
      {
        ...
        specialtyIds: [],
        requiresApproval: true,
        autoApprove: true, // Tự động duyệt
        moderatorIds: [],
        isActive: true,
        icon: 'FaAppleAlt',
        color: '#f39c12',
      ...
      },
      {
        ...
        slug: 'tim-mach',
        specialtyIds: [1], // Specialty ID tim mạch
        requiresApproval: true,
        autoApprove: false,
        moderatorIds: [],
        isActive: true,
    ...
    const count = await Topic.count({ transaction });
